  ...
  return (
    <div className="p-8">
      ...
      {worker.positions.length === 0 && (
        <div className="bg-navy-500/10 border border-gold-300/20 rounded-xl p-6 mb-8">
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1">
              ...
              <Link
                ...
                className="inline-block bg-navy-600 hover:bg-navy-500 text-navy-600 px-6 py-3 rounded-lg font-semibold transition-all"
              >
      ...
      {worker.positions.length > 0 && (
        <div className="mb-8">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-xl font-semibold text-navy-600">Your Positions</h2>
            <Link
          ...
          {worker.positions.length === 0 && (
            <div className="text-center py-12 bg-white rounded-xl">
              <p className="text-soft-400 mb-4">No positions yet</p>
              <Link
                href="/dashboard/positions/new"
                className="text-navy-500 hover:text-navy-400 transition-colors"
              >
                Add your first position →
              </Link>
            </div>
      ...
      {recentReviews && recentReviews.length > 0 && (
        <div>
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-xl font-semibold text-navy-600">Recent Reviews</h2>
            <Link
...
function StatCard({ 
  ...
  return (
    <div className="bg-white border border-soft-200 rounded-xl p-6">
      ...
      <div className="text-sm text-soft-500">{label}</div>
      <div className="text-xs text-soft-400 mt-1">{subtext}</div>
    </div>
  )
}
...
function PositionCard({ position }: { position: any }) {
  ...
  return (
    ...
    >
      ...
      <div className="text-xs text-soft-400 mt-3">{dateRange}</div>
    </Link>
  )
}
...
function ReviewCard({ review }: { review: any }) {
  ...
  return (
    <div className="bg-white border border-soft-200 rounded-xl p-6">
      ...
      {review.comment && (
        <p className="text-soft-500 italic">"{review.comment}"</p>
      )}
      {review.reviewer_name && (
        <p className="text-xs text-soft-400 mt-2">— {review.reviewer_name}</p>
      )}
    </div>
  )
}
