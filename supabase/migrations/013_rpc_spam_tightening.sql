-- ============================================================
-- Tighten anon-callable RPCs against spam
-- ============================================================

-- increment_qr_scan: return boolean, do nothing if token does not exist
...
-- log_verification_event: require p_actor_type in ('user','token');
-- when p_actor_type='token', require metadata contains { "token_based": true }
...
BEGIN
  ...
  IF p_actor_type = 'token' THEN
    IF COALESCE((p_metadata->>'token_based')::boolean, false) IS NOT TRUE THEN
      RAISE EXCEPTION 'metadata must contain token_based: true when actor_type is token';
    END IF;
  END IF;
